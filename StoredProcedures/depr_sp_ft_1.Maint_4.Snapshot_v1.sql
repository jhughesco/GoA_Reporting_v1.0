...
CREATE OR ALTER PROCEDURE dbo.[sp_ft_1.Maint_4.Snapshot]
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 1. Purge FIRST to reduce table size (retain where startDT >= cutoff)
        DELETE FROM dbo.[ft_Metadata_Snapshot]
        WHERE startDT < @retention_cutoff;
        
        SET @deleted_count = @@ROWCOUNT;

        -- 2. Insert only NEW records for relevant date range
        ...
        WHERE v.startDT >= @retention_cutoff
          AND NOT EXISTS (
              SELECT 1 
              FROM dbo.[ft_Metadata_Snapshot] s 
              WHERE s.stream_id = v.stream_id
                AND s.startDT = v.startDT -- Critical for retention logic
          );
